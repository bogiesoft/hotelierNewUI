...
if ($action == "check_available"){
		...
		foreach($roomtypes_suitable as $roomtype_suitable) {
			...
			array_push($roomtypeCount,count($roomsIdentify_free));
...
function get_roomsID_free($roomsIdentify_onDuty,$propertyID, $arrival_date,$departure_date,$check_time) {
	...
	//Loop for the rooms that should be checked
	 ...
	 foreach ($rooms_to_check as $room_ch) {
		...
		if ($new_firstD_day == "1" && $room_status == "free") {
			...
			$free_rooms[] = get_roomsID_free($room_ch,$propertyID, $new_arrival_date,$new_departure_date,2)[0];
		} else {
			if ($room_status == "free") {
				$free_rooms[] = $room_ch;
			}
