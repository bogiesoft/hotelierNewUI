...
$action = $data["action"];
$token = isset($data["token"]) ? $data["token"] : 'dummyToken' ;
$email = isset($data["email"]) ? $data["email"] : 'dummyEmail' ;
...
if ($action == "check_available"){
		...
		foreach($roomstypeID_avail as $roomtypeID) {
			...
			$i++;
			$roomtype_data = get_roomtype_details($roomtypeID, $quantity, $arrival_date,$departure_date);
...
function get_roomsID_free($roomsIdentify_onDuty,$propertyID, $arrival_date,$departure_date) { //$firstNight, $lastNight, $roomQty, $category_rooms, $conn, $prefix
	...
	// foreach($roomsIdentify_onDuty as $roomIdentify_onDuty) {
	//
	// 	echo $roomIdentify_onDuty;
	//
	// }
...
function get_roomtype_details($roomtypeID,$quantity,$arrival_date,$departure_date) {
...
function get_roomtype_availability($roomtypeID,$start_date,$end_date) {
...
function areRoomsFree($firstNight, $lastNight, $roomQty, $category_rooms, $conn, $prefix) {

  $rooms_in_booking = array();
  $free_rooms = array();
  $rooms_to_check = array();

  $sql = "SELECT * FROM " . $prefix . "apb_availability"; //to add property id
  $result = $conn->query($sql);
